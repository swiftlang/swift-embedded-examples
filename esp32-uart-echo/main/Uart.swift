...
struct Uart {
    ...
    init(portNum: Int = 1,
        ...
        guard uart_driver_install(self.portNum, Int32(bufferSize * 2), 0, 0, nil, 0) == ESP_OK else {
        ...
        guard set_uart_pin(self.portNum,
                                 Int32(txPin),
                                 Int32(rxPin),
                                 UART_PIN_NO_CHANGE,
                                 UART_PIN_NO_CHANGE) == ESP_OK else {
    ...
    func write(_ data: UnsafePointer<UInt8>, length: Int) -> Int {
        return Int(uart_write_bytes(portNum, data, length))
    }
    ...
    func read(into buffer: UnsafeMutablePointer<UInt8>, maxLength: Int, timeoutMs: UInt32) -> Int {
